...
export function handlePairCreated(event: PairCreatedEvent): void {
  ...
  if (token0 === null) {
    ...
    token0.glmrPrice = ZERO_BD;
    token0.liquidity = ZERO_BD;
    token0.liquidityInUsd = ZERO_BD;
    token0.liquidityInGlmr = ZERO_BD;
  ...
  if (token1 === null) {
    ...
    token1.glmrPrice = ZERO_BD;
    token1.liquidity = ZERO_BD;
    token1.liquidityInUsd = ZERO_BD;
    token1.liquidityInGlmr = ZERO_BD;
...
export function fetchTokenSymbol(token: Address): string {
  ...
  // DEV token symbol with ce
  if (
    (token == usdt_address || token == usdc_address || token == weth_address) &&
    subgraphMode == SUBGRAPH_MODE.GET_TOKEN_DETAIL_FROM_PROD
  ) {
    symbol = "ce" + symbol;
  }

  if (token == weth_address && subgraphMode == SUBGRAPH_MODE.GET_TOKEN_DETAIL_FROM_DEV) {
    symbol = "ceW" + symbol;
  }

  if (token == busd_address || token == wbtc_address) {
    symbol = "ce" + symbol;
  }
  return symbol;
}
...
export function fetchPairForTokenAddress(
  token0: Address,
  token1: Address
): Address {
